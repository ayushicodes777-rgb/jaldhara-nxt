...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({
  ...
  const fetchWeather = async (loc: string) => {
    ...
    try {
      ...
      if (data.error) {
        toast.info(
          language === "en"
            ? "Using estimated weather data due to API issues."
            : "API समस्याओं के कारण अनुमानित मौसम डेटा का उपयोग कर रहे हैं।",
        );
      } else {
        toast.success(
          language === "en"
      ...
      toast.error(
        language === "en"
          ? "Weather service temporarily unavailable. Using estimated data."
          : "मौसम सेवा अस्थायी रूप से अनुपलब्ध है। अनुमानित डेटा का उपयोग कर रहे हैं।",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const getWeatherIcon = (condition: string) => {
    ...
    if (lowerCondition.includes("sun") || lowerCondition.includes("clear")) {
      ...
      return (
        <CloudRain className="h-12 w-12 text-blue-500 animate-weather-rain" />
      );
    } else if (
      ...
      return (
        <CloudFog className="h-12 w-12 text-gray-500 animate-weather-fog" />
      );
    } else if (lowerCondition.includes("wind")) {
      return <Wind className="h-12 w-12 text-teal-500 animate-weather-wind" />;
    } else {
      return (
        <Cloud className="h-12 w-12 text-blue-400 animate-weather-float" />
      );
    }
  };
        ...
        <CardHeader className="bg-gradient-to-r from-blue-100 to-sky-100 dark:from-blue-950 dark:to-sky-950">
          <CardTitle className="text-lg flex justify-between items-center">
            <span className="font-medium text-blue-800 dark:text-blue-200">
              {language === "en" ? "Weather Information" : "मौसम की जानकारी"}
            </span>
            {weather && (
              <ChevronRight className="h-4 w-4 text-blue-700 dark:text-blue-300 group-hover:translate-x-1 transition-transform" />
            )}
          </CardTitle>
        </CardHeader>
